...
export async function loadNorthwest93({scene,renderer,sample,variant}){
 ...
 const road=(await loader.loadAsync('/67park-island/island/northwest-v93/track.glb?v=93.2')).scene;
 ...
 for(const p of [{asset:'blush',x:-141,z:-128},{asset:'ivory',x:-141,z:-113},{asset:'annex',x:-141,z:-99}]){
  const root=(await loader.loadAsync('/67park-island/island/northwest-v93/'+p.asset+'.glb?v=93.3')).scene;root.name='NW93 detached '+p.asset;root.position.set(p.x,ground(p.x,p.z)-.012,p.z);root.rotation.y=Math.PI/2;finish(root);group.add(root);root.updateWorldMatrix(true,true);
