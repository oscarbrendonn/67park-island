...
export async function loadNorthwest92({scene,renderer,sample,variant}){
 ...
 const road=(await loader.loadAsync('/67park-island/island/northwest-v92/track.glb?v=92.2')).scene;
 ...
 for(const p of [{asset:'blush',x:-141,z:-128},{asset:'ivory',x:-141,z:-113},{asset:'annex',x:-141,z:-99}]){
  const root=(await loader.loadAsync('/67park-island/island/northwest-v92/'+p.asset+'.glb?v=92.3')).scene;root.name='NW92 detached '+p.asset;root.position.set(p.x,ground(p.x,p.z)-.012,p.z);root.rotation.y=Math.PI/2;finish(root);group.add(root);root.updateWorldMatrix(true,true);
 ...
 const tree=(await loader.loadAsync('/67park-island/island/northwest-v92/tree.glb?v=92')).scene;
