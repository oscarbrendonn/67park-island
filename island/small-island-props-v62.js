...
async function loadSmallIslandPropsUncached({scene,renderer,sample,variant},foliageShared){
 ...
 const [houseGltf,foliageGltf,layout]=await Promise.all([
  new GLTFLoader().loadAsync('/67park-island/island/small-island-houses-v62.glb?v=roof1'),
  foliageShared,
...
export function loadSmallIslandProps(options){
 return withIslandFoliage(options.assetSession,()=>new GLTFLoader().loadAsync('/67park-island/island/small-island-props-v45.glb?v=round1'),foliage=>loadSmallIslandPropsUncached(options,foliage));
}
