...
export function createCandyVehicle110({id='main-mint',color='#8ebfb5',kind='car'}={}){
 ...
 box(fixed,.31,.13,.045,'rubber',-.61,bus?1.54:1.18,dashboardZ-.06,.019);
 const seatRows=bus?[1.80,.66,-.48,-1.62]:[.11,-1.12],seats=[];
 for(const z of seatRows)for(const x of [-.61,.61]){
  ...
  seats.push({index:seats.length,role:seats.length?'passenger':'driver',group:sg,footRest:fr,position:base.clone(),base,rig:null});
 }
 batch(fixed);
 const wheels=[];
 ...
 const steering=new T.Group();steering.position.set(-.61,bus?1.50:1.12,bus?2.10:.47);steering.rotation.x=-.3;model.add(steering);
 ...
 const end=new T.Vector3(-.61,bus?1.49:1.13,dashboardZ),delta=end.clone().sub(steering.position);column.position.copy(steering.position).add(end).multiplyScalar(.5);column.scale.y=delta.length();column.quaternion.setFromUnitVectors(new T.Vector3(0,1,0),delta.normalize());
 const textures=[];
